...
//import { getUserDetails, updateStudentFields, getSubjectList } from '../../../redux/userRelated/userHandle';
...
const StudentAttendance = ({ situation }) => {
    ...
    const submitHandler = (e) => {
        ...
        if (!formData.chosenSubName || !formData.status || !formData.date) {
        ...
        setLoader(true);
        dispatch(updateStudentFields(formData.studentID, fields, "StudentAttendance"));
    };
    ...
    useEffect(() => {
        if (response) {
            ...
            setLoader(false);
            setAlert({ show: true, type: 'error', message: "An error occurred" });
        }
        else if (statestatus === "added") {
            setLoader(false);
            setAlert({ show: true, type: 'success', message: "Attendance recorded successfully!" });
        }
                ...
                <div className="p-8">
                    ...
                    <form onSubmit={submitHandler} className="space-y-6">
                        ...
                        <button
                            type="submit"
                            disabled={loader}
                            className={`w-full py-3 px-4 rounded-lg font-medium text-white transition-all duration-200 ${loader ? 'bg-purple-400 cursor-not-allowed' : 'bg-purple-600 hover:bg-purple-700'} focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2`}
                        >
                            {loader ? (
